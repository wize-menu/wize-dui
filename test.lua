...
local function getUserKey()
    -- Assuming MachoMenuInput is a custom function that prompts for text input and returns it
    -- Replace with your framework's input method, e.g., NUI or native UI
    local userInput = MachoMenuInput("Enter your product key:", 50) -- 50 is max length
...
local CurrentKey = getUserKey()

-- Fallback if no input method is available: prompt via chat command
if not CurrentKey then
    MachoMenuNotification("WizeMenu", "Please use /enterkey <your-key> to submit a key.", 10)
    RegisterCommand("enterkey", function(source, args)
        if args[1] then
            CurrentKey = args[1]
            MachoMenuNotification("WizeMenu", "Key submitted: " .. CurrentKey, 5)
        else
            MachoMenuNotification("WizeMenu", "No key provided. Use /enterkey <your-key>", 10)
        end
    end, false)
end
...
-- Check if key is valid; if not, exit
if not CurrentKey or not isKeyValid() then
    MachoMenuNotification("WizeMenu", "Your key ain't valid lmfao: " .. (CurrentKey or "No key provided"), 10)
    return
end
